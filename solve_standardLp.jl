function solve_standardlp(A,b,c,maxit=100,tol=1e-8,verbose=false)
    ...
    for iter=1:maxit
        ...
        # alpha_max_pri = isfinite(alpha_max_pri) ? alpha_max_pri : 1.0
        # alpha_max_dual = isfinite(alpha_max_dual) ? alpha_max_dual : 1.0       
        ...
        if scaling == 0
            ...
            sind = argmin(s1_dual)
            # @test s1_dual[sind] == 0
            # tol_x = 1e-12  # a small tolerance for the primal variable
            # if abs(x1_pri[sind]) < tol_x || abs(ds[sind]) < tol_x
            #     f_dual = 1 - gamma_f
            # else
        ...
        @show alpha_max_pri, alpha_max_dual
        @show alpha_pri, alpha_dual

        # Check for unboundedness - but only if there's a non-zero direction and objective is improving
